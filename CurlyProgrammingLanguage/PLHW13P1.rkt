...
(define (instanceof? [classes : (Listof (Symbol * Class))] [curr : Symbol] [target : Symbol] ) : Value
   
   (if (equal? curr 'Object)
       (numV 1)
       (if (equal? curr target)
           (numV 0)
           (instanceof? classes (classC-super-name (find classes curr)) target )
        )
    )
  
 )

(define (createBoxesList [v : (Listof (Boxof Value))] [e : (Listof Exp)]
                         [classes : (Listof (Symbol * Class))] [this : Value] [arg : Value]) : (Listof (Boxof Value))

  (if (empty? e)
        v
        (createBoxesList
          (cons (box (interp (first e) classes this arg)) v)
         (rest e) classes this arg)
     )
 )

(define (getValue [v : (Listof (Boxof Value))] [s : (Listof Symbol)] [wanted : Symbol] ) : Value

  (if (empty? s)
        (error 'interp "field not found")
        
        (if (equal? wanted (first s))
            (unbox (first v))
            (getValue (rest v) (rest s) wanted)
         )
     )
 )

(define (setValue [newVaList : (Listof (Boxof Value))] [v : (Listof (Boxof Value))] [s : (Listof Symbol)] [wanted : Symbol]
                  [newVal : Value] ) : (Listof (Boxof Value))

  (if (empty? s)
        newVaList
        
        (if (equal? wanted (first s))
            (setValue (cons  (box newVal) newVaList) (rest v) (rest s) wanted newVal)
            (setValue (cons (first v) newVaList) (rest v) (rest s) wanted newVal)
         )
     )
 )
  ...
  (lambda (a classes this-val arg-val)
    (local [(define (recur expr)
      ...
      (type-case Exp a
        ...
        [(argE) arg-val]
         ...
         (if (equal? class-name 'Object)
                ...
                (local [ (define c (find classes class-name))]
                     (if (= (length field-exprs) (length (classC-field-names c)))
                         (objV class-name
                           (createBoxesList empty field-exprs classes this-val arg-val)
                          )
        ...
        [(castE s e)
           (local [(define obj (recur e)) ]
            (if (objV? obj)
                (if (equal? 'Object s)
                  obj
                  (if (equal? 0 (numV-n (instanceof? classes(objV-class-name obj) s )))
                      obj
                      (error 'interp "not a subtype")
                   )
                 )
                (error 'interp "wrong types")
             )
          )
         ]

        [(if0E c t f) (local [(define condition (recur c))]
                   (if (numV? condition)
                        (if (equal? 0 (numV-n condition) )
                            (recur t)
                            (recur f)
                        )
                       (error 'interp "condition not a number")
                    )
                  )
       ]


     [(setE obj-expr field-name new-expr)
         (local [(define new-val (recur new-expr))]
           (if (nullV? new-val)
               (error 'interp "no null fields")

                (type-case Value (recur obj-expr)
           [(objV class-name field-vals)
            (type-case Class (find classes class-name)
              [(classC super field-names methods)
                 (objV class-name (setValue empty field-vals field-names field-name new-val))
               ]
              )]
        ...
        [(getE obj-expr field-name)
         (type-case Value (recur obj-expr)
           [(objV class-name field-vals)
            (type-case Class (find classes class-name)
              [(classC super field-names methods)
                 (getValue field-vals field-names field-name)
                ]
              )]
  ...
  (type-case Class (find classes class-name)
    [(classC super field-names methods)
     (let ([body-expr (find methods method-name)])
       ...
       (local [(define return (interp body-expr
               classes
               obj
               arg-val))]
            (if (nullV? return)
                (error 'interp "null method")
                return
             )
         )
...
(define (exp-i->c [a : ExpI] [super-name : Symbol]) : Exp
  (local [(define (recur expr)
    ...
    (type-case ExpI a
      ...
      [(setI e1 s e2) (setE (recur e1) s (recur e2))]
  ...
  (type-case Class (find classes-not-flat name)
    [(classC super field-names methods)
     (type-case Class (flatten-super name classes-not-flat i-classes)
       [(classC superr super-field-names super-methods)
        (classC
         super
...
(define (parse [s : S-Exp]) : ExpI
  (cond
    ...
    [(s-exp-match? `{new SYMBOL ANY ...} s)
     ...
     [(s-exp-match? `{cast SYMBOL ANY} s) (castI
